package com.example.photoeditor.splines.model
            ...
            when (arrSegments[i].size) {
                4 -> {
                    while (t <= 1.0) {
                        ...
                        arrSplines.add(arrSplines.size,CoordinatePoints(xS.toFloat(),yS.toFloat()))
                    }
                }
                3 -> {
                    if (i != 0) {
                    ...
                    while (t <= 1.0) {
                        t += 0.05
                        xS =
                            (1.0 - t).pow(2.0) * arrSegments[i][0].x + 2.0 * t * (1.0 - t) * arrSegments[i][1].x + t.pow(
                                2.0
                            ) * arrSegments[i][2].x
                        yS =
                            (1.0 - t).pow(2.0) * arrSegments[i][0].y + 2.0 * t * (1.0 - t) * arrSegments[i][1].y + t.pow(
                                2.0
                            ) * arrSegments[i][2].y
                        arrSplines.add(arrSplines.size,CoordinatePoints(xS.toFloat(),yS.toFloat()))
                    }
                ...
                1 -> {
                    ...
                    arrSplines.add(
                        arrSplines.size, CoordinatePoints(arrSegments[i][0].x,arrSegments[i][0].y)
                    )
                }
            }
        }
